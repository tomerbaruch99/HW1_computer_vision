...
def predict_image(image_path, model, classid_classname):
    image = cv2.imread(image_path)
    results = model(image)
    for result in results:
        for box in result.boxes:
            x1, y1, x2, y2 = [int(box.xyxy[0][i].item()) for i in range(4)]
            cv2.rectangle(image, (x1, y1), (x2, y2), (102, 0, 204), 2)
            class_id = int(box.cls.item())
            confidence = float(box.conf.item())
            cv2.putText(image, f'{classid_classname[class_id]} , {confidence:.2f}', (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (102, 0, 204), 2)
...
if __name__ == '__main__':
    ...
    predict_image(image_path, model, classid_classname)